  ...
  config = mkIf cfg.enable {
    assertions = mapAttrsToList
      (name: value: {
        ...
        message = "nixgehtmehr.nix.extra-substituters.${name}.key must be set";
      })
      cfg.extra-substituters;

    environment.systemPackages = with pkgs; [
      nixgehtmehr.nixos-revision
      (nixgehtmehr.nixos-hosts.override { hosts = inputs.self.nixosConfigurations; })
      deploy-rs
      nixfmt-rfc-style
      nix-index
      nix-prefetch-git
